...
_EFFECTS = {
    ...
    "appear":     (1,  None),
    "wipe":       (22, "wipe(up)"),
    "wipe_right": (22, "wipe(right)"),
    "wipe_down":  (22, "wipe(down)"),
    "wipe_left":  (22, "wipe(left)"),
    "fly_up":     (2,  "fly(fromBottom)"),
    "fly_left":   (2,  "fly(fromLeft)"),
    "fly_right":  (2,  "fly(fromRight)"),
}
...
class Build:
    ...
    def __init__(self, slide):
        ...
        self.steps = []
        self.step_triggers = []  # ('click', 0) or ('auto', delay_ms)
        self._id = 3
    ...
    def step(self, auto=False, delay_ms=0):
        """Draw shapes to reveal on click, or auto after delay_ms when auto=True."""
        ...
        if spids:
            ...
            self.step_triggers.append(("auto", delay_ms) if auto else ("click", 0))
    ...
    def _step_xml(self, spids, preset_id, filt, dur_ms, trigger="click", delay_ms=0):
        node0 = "afterEffect" if trigger == "auto" else "clickEffect"
        cond = f'delay="{delay_ms}"' if trigger == "auto" else 'delay="indefinite"'
        effects = "".join(
            self._effect_xml(spid, node0 if i == 0 else "withEffect", preset_id, filt, dur_ms)
            for i, spid in enumerate(spids)
        )
        inner_cond = f'<p:stCondLst><p:cond {cond}/></p:stCondLst>'
        return (
            f'<p:par><p:cTn id="{self._nid()}" fill="hold">'
            f'{inner_cond}<p:childTnLst>'
            f'<p:par><p:cTn id="{self._nid()}" fill="hold">'
    ...
    def apply(self, effect="fade", duration=0.5, effects=None):
        """Write timing XML. effects: optional list of effect names per step."""
        if not self.steps:
            return 0
        dur_ms = int(duration * 1000)
        steps_xml_parts = []
        for i, spids in enumerate(self.steps):
            eff = (effects[i] if effects and i < len(effects) else effect)
            if eff not in _EFFECTS:
                raise ValueError(f"unknown effect {eff!r}")
            preset_id, filt = _EFFECTS[eff]
            trigger, delay_ms = self.step_triggers[i] if i < len(self.step_triggers) else ("click", 0)
            steps_xml_parts.append(self._step_xml(spids, preset_id, filt, dur_ms, trigger, delay_ms))
        steps_xml = "".join(steps_xml_parts)
        xml = (
